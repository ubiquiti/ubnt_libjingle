...
H265VpsSpsPpsTracker::FixedBitstream H265VpsSpsPpsTracker::CopyAndFixBitstream(
  ...
  //RTC_LOG(LS_WARNING) << "#-> H265VpsSpsPpsTracker::" << __func__ ;
  auto& h265_header =
  ...
  for (size_t i = 0; i < h265_header.nalus_length; ++i) {
    ...
    switch (nalu.type) {
      case H265::NaluType::kVps: {
        RTC_LOG(LS_WARNING) << "    H265VpsSpsPpsTracker::" << __func__ << " VPS     vps_id=" << nalu.vps_id;
        vps_data_[nalu.vps_id].size = 0;
        break;
      }
      case H265::NaluType::kSps: {
        RTC_LOG(LS_WARNING) << "    H265VpsSpsPpsTracker::" << __func__ << " SPS     vps_id=" << nalu.vps_id << " sps_id=" << nalu.sps_id << " width=" << video_header->width << " height=" << video_header->height;
      ...
      case H265::NaluType::kPps: {
        RTC_LOG(LS_WARNING) << "    H265VpsSpsPpsTracker::" << __func__ << " PPS     sps_id=" << nalu.sps_id << " pps_id=" << nalu.pps_id;
      ...
      case H265::NaluType::kCra: {
        ...
        if (video_header->is_first_packet_in_frame) {
          if (nalu.pps_id == -1) {
            RTC_LOG(LS_WARNING) << "<-# H265VpsSpsPpsTracker::" << __func__ << " FAILED! No PPS id in IDR nalu. REQUEST KEY FRAME";
            return {kRequestKeyframe};
          }

          //RTC_LOG(LS_WARNING) << "    H265VpsSpsPpsTracker::" << __func__ << " key frame with pps_id=" << nalu.pps_id;

          // Karlis ############################################################
          pps = pps_data_.find(0/*nalu.pps_id*/);
          if (pps == pps_data_.end()) {
            RTC_LOG(LS_WARNING) << "<-# H265VpsSpsPpsTracker::" << __func__ << " FAILED! No PPS with id " << nalu.pps_id << " received. REQUEST KEY FRAME";
            return {kRequestKeyframe};
          }

          sps = sps_data_.find(pps->second.sps_id);
          if (sps == sps_data_.end()) {
            RTC_LOG(LS_WARNING) << "<-# H265VpsSpsPpsTracker::" << __func__ << " FAILED! No SPS with id << " << pps->second.sps_id << " received. REQUEST KEY FRAME";
            return {kRequestKeyframe};
          }

          vps = vps_data_.find(sps->second.vps_id);
          if (vps == vps_data_.end()) {
            RTC_LOG(LS_WARNING) << "<-# H265VpsSpsPpsTracker::" << __func__ << " FAILED! No VPS with id " << sps->second.vps_id << " received. REQUEST KEY FRAME";
            return {kRequestKeyframe};
          }
  ...
  // Karlis ############################################################
  if (h265_header.packetization_type == kH265FU && !video_header->is_first_packet_in_frame) {
    append_vps_sps_pps = false;
  }
  ...
  //RTC_LOG(LS_WARNING) << "<-# H265VpsSpsPpsTracker::" << __func__ << " OK. INSERT";
  return fixed;
}
