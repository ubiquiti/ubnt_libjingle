...
absl::optional<VideoRtpDepacketizer::ParsedRtpPayload> ProcessApOrSingleNalu(
  ...
  if (nal_type == H265::NaluType::kAP) {
    ...
    //RTC_LOG(LS_ERROR) << "    ProcessApOrSingleNalu() nalu_type=" << nal_type << " nalu_length=" << nalu_length << " ------------------ AGREGATE ------------------rtp_payload.size()=" << rtp_payload.size();
    ...
    if (!ParseApStartOffsets(nalu_start, nalu_length, &nalu_start_offsets)) {
      RTC_LOG(LS_ERROR) << "ProcessApOrSingleNalu() FAILED! AP packet with incorrect NALU packet lengths";
      return absl::nullopt;
    }
    ...
    //RTC_LOG(LS_ERROR) << "    ProcessApOrSingleNalu() nalu_type=" << nal_type << " nalu_length=" << nalu_length << " ------------------ SINGLE ------------------rtp_payload.size()=" << rtp_payload.size();
  ...
  for (size_t i = 0; i < nalu_start_offsets.size() - 1; ++i) {
    ...
    switch (nalu.type) {
      case H265::NaluType::kVps: {
        ...
        //RTC_LOG(LS_ERROR) << "    ProcessApOrSingleNalu() nalu_type=" << nal_type << " nalu_length=" << nalu_length << " ------------------ VPS vps_id=" << nalu.vps_id << " ------------------";
      ...
      case H265::NaluType::kSps: {
        ...
        if (sps) {
          ...
          RTC_LOG(LS_WARNING) << "ProcessApOrSingleNalu() Failed to parse SPS and VPS id from SPS slice #######################################";
        }

        //RTC_LOG(LS_ERROR) << "    ProcessApOrSingleNalu() nalu_type=" << nal_type << " nalu_length=" << nalu_length << " ------------------ SPS vps_id=" << nalu.vps_id << " sps_id=" << nalu.sps_id << " ------------------";
      ...
      case H265::NaluType::kPps: {
        ...
        if (H265PpsParser::ParsePpsIds(&payload_data[start_offset],
          ...
          nalu.pps_id = pps_id;
          nalu.sps_id = sps_id;
          //RTC_LOG(LS_ERROR) << "    ProcessApOrSingleNalu() nalu_type=" << nal_type << " nalu_length=" << nalu_length << " ------------------ PPS pps_id=" << nalu.pps_id << " sps_id=" << nalu.sps_id << "------------------";
        } else {
      ...
      case H265::NaluType::kCra:
        //RTC_LOG(LS_ERROR) << "    ProcessApOrSingleNalu() nalu_type=" << nal_type << " nalu_length=" << nalu_length << " ------------------ KEY FRAME ------------------";
        parsed_payload->video_header.frame_type = VideoFrameType::kVideoFrameKey;
...
absl::optional<VideoRtpDepacketizer::ParsedRtpPayload> ParseFuNalu(
  ...
  // Karlis
  bool end_fragment = rtp_payload.cdata()[2] & kHevcEBit;
  ...
  //RTC_LOG(LS_ERROR) << "    ParseFuNalu() nalu_type=" << original_nal_type << " ------------------ FU ------------------ rtp_payload.size()=" << rtp_payload.size() << " first_fragment=" << first_fragment << " end_fragment=" << end_fragment;
