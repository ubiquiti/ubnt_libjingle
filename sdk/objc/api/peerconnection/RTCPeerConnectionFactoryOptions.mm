...
@synthesize activeInterfaces = _activeInterfaces; // this is not needed since some prehistoric XCode version, should be autogenerated now
// UI Customization End
- (instancetype)init {
  return [super init];
}
// UI Customization Begin
- (void)setActiveInterfaces:(NSArray *)activeInterfaces {
    _activeInterfaces = [activeInterfaces copy];
}
// UI Customization End
...
// UI Customization Begin
  options.activeInterfaces.clear();
  if(self.activeInterfaces!=nil) {
    for (NSString *interface in self.activeInterfaces)
      options.activeInterfaces[[interface UTF8String]] = true;
  }
// UI Customization End
  return options;
}
